class Table {
  ...
  constructor(state, setGlobalState) {
    ...
    const slimmedData = state.data.map(d => ({
      "State": d.State,
      "COVID Total Cases": d['COVID Total Cases']
    })).sort((a, b) => d3.descending(a['COVID Total Cases'], b['COVID Total Cases']))
    ...
    const logScale = d3
      ...
      .domain(d3.extent(slimmedData, d => d['COVID Total Cases']))
      .range([0.5, 1]); // to use only the darker half of the color scale

    // use that logarithmic scale in our color interpolator
    this.colorScale = d3.scaleSequential(d => d3.interpolateSinebow(logScale(d)));

    const columns = ["State", "COVID Total Cases"];
    ...
    this.tableRows = table
      ...
      .data(slimmedData)
      .join("tr")
      .style("background-color", d => this.colorScale(d['COVID Total Cases']))
      .style("color", "#eee");
  ...
  draw(state, setGlobalState) {
    ...
    this.tableRows.style("background-color", d =>
      state.selectedState === d.State ? "gray" : this.colorScale(d['COVID Total Cases'])
    );
  }
}
